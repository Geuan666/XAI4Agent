#!/usr/bin/env python3
"""Build agentic/assisted prompts + aligned tokens for gpt-oss-20b.
...
DEFAULT_MODEL = "/root/autodl-tmp/models/gpt-oss-20b"
DEFAULT_PARQUET = "/root/autodl-tmp/xai/dataset/humaneval/data/test-00000-of-00001.parquet"
DEFAULT_AGENTIC = "/root/autodl-tmp/xai/model/gpt-oss-20b/agentic_output.json"
DEFAULT_ASSISTED = "/root/autodl-tmp/xai/exp/assisted/assisted_output.json"
DEFAULT_OUT_DIR = "/root/autodl-tmp/xai/model/gpt-oss-20b"
...
ASSISTANT_TAG = "<|start|>assistant"
IM_END = "<|end|>"
SYSTEM_START = "<|start|>system"
USER_START = "<|start|>user"
DEVELOPER_START = "<|start|>developer"

# gpt-oss tool call/response markers in prompt
TOOL_CALL_OPEN = "<|start|>assistant to=functions.read_file"
TOOL_CALL_CLOSE = "<|call|>"
TOOL_RESPONSE_OPEN = "<|start|>functions.read_file to=assistant"
TOOL_RESPONSE_CLOSE = "<|end|>"

TOOLS_BLOCK_OPEN = "# Tools"
TOOLS_BLOCK_CLOSE = "} // namespace functions"

CODE_FENCE_PATTERNS = ("```python", "```py", "```")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build prompts/tokens for gpt-oss-20b.")
...
def find_segment_range(
    ...
    if idx == -1 and allow_escaped:
        escaped = json.dumps(segment, ensure_ascii=False)[1:-1]
        idx = text.find(escaped, start_char, end_char)
        if idx == -1:
            return -1, -1, -1, -1
        segment = escaped
    if idx == -1:
...
def main() -> int:
    ...
    if pad_token_id is None and hasattr(tokenizer, "token_to_id"):
        ...
        if pad_token_id is None:
            pad_token_id = tokenizer.token_to_id("<|end|>")
    if pad_token_id is None:
        pad_token_id = getattr(tokenizer, "eos_token_id", None)
    if pad_token_id is None:
        cfg_path = Path(args.model_path) / "tokenizer_config.json"
        if cfg_path.exists() and hasattr(tokenizer, "token_to_id"):
            cfg = json.loads(cfg_path.read_text())
            for key in ("pad_token", "eos_token"):
                tok = cfg.get(key)
                if tok:
                    pad_token_id = tokenizer.token_to_id(tok)
                    if pad_token_id is not None:
                        break
    if pad_token_id is None:
        raise RuntimeError("Tokenizer has no pad/eos token id")
    ...
    for row in rows:
        ...
        if segments is None:
            ...
            tool_resp_open_char, tool_resp_open_char_end, _, _ = find_marker_range_any(
                agentic_full, [TOOL_RESPONSE_OPEN], agentic_offsets
            )
            tool_resp_close_char, _, _, _ = find_marker_range_any(
                agentic_full, [TOOL_RESPONSE_CLOSE], agentic_offsets, tool_resp_open_char
            )
            for name, text in segments.items():
                ...
                char_start, char_end, tok_start, tok_end = find_segment_range(
                    agentic_full, seg_text, agentic_offsets, search_start, search_end, allow_escaped=True
                )
                if tok_start < 0:
                    trimmed = seg_text.rstrip()
                    if trimmed and trimmed != seg_text:
                        char_start, char_end, tok_start, tok_end = find_segment_range(
                            agentic_full,
                            trimmed,
                            agentic_offsets,
                ...
                seg_ids = encode(tokenizer, seg_text)
                found = tok_start >= 0
                segment_meta[name] = {
        ...
        system_char_start, _, system_start, system_start_end = find_marker_range_any(
            agentic_full, [SYSTEM_START], agentic_offsets
        )
        ...
        user_char_start, _, user_start, user_start_end = find_marker_range_any(
            agentic_full, [USER_START], agentic_offsets, system_char_end
        )
        ...
        _, _, assistant_start, assistant_end = find_marker_range_any(
            agentic_full, [ASSISTANT_TAG], agentic_offsets
        )

        # tool positions (gpt-oss specific)
        _, _, tools_start, tools_end = find_marker_range_any(
            agentic_full, [TOOLS_BLOCK_OPEN], agentic_offsets
        )
        _, _, tools_close_start, tools_close_end = find_marker_range_any(
            agentic_full, [TOOLS_BLOCK_CLOSE], agentic_offsets
        )

        # There is no explicit example tool_call block; treat first as actual.
        ...
        _, _, tool_call_close_start, tool_call_close_end = find_marker_range_any(
            agentic_full, [TOOL_CALL_CLOSE], agentic_offsets, tool_call_start
        )
        ...
        _, _, tool_response_close_start, tool_response_close_end = find_marker_range_any(
            agentic_full, [TOOL_RESPONSE_CLOSE], agentic_offsets, tool_response_char_start
        )
        ...
        tokens_out[key] = {
            ...
            "tool_positions": {
                ...
                "tool_call_example_open": {"start": -1, "end": -1},
                "tool_call_example_close": {"start": -1, "end": -1},
                "tool_call_actual_open": {"start": tool_call_start, "end": tool_call_end},
                "tool_call_actual_close": {"start": tool_call_close_start, "end": tool_call_close_end},
                ...
                "important_open": {"start": -1, "end": -1},
                "important_close": {"start": -1, "end": -1},
            },
            "message_positions": {
