# Nemotron-3-Nano-30B-A3B prompt/token builder

## Files
- `vllm_nemotron_3_nano_30b_a3b.sh`
  - vLLM 启动脚本（工具调用解析器：`qwen3_coder`）
  - 默认端口：8002
...
1) 启动 vLLM
```bash
nohup bash /root/autodl-tmp/xai/model/Nemotron-3-Nano-30B-A3B/vllm_nemotron_3_nano_30b_a3b.sh \
...
3) 生成 prompts/tokens
```bash
/root/miniconda3/envs/qwen/bin/python /root/autodl-tmp/xai/model/Nemotron-3-Nano-30B-A3B/build_prompt_tokens.py
```

## Tool-call format
- 使用 **OpenAI tools schema**（OpenAI compatible）
