...
DEFAULT_PROJECT_ROOT = "/root/autodl-tmp/XAI4Agent/xai4agent/pipelines/real/projects"
DEFAULT_RUN_ROOT = "/root/autodl-tmp/XAI4Agent/experiments/real"
AGENT_MODULE_PATH = "/root/autodl-tmp/XAI4Agent/xai4agent/pipelines/real/agent.py"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Agentic coding run (write/run) for HumanEval.")
...
def resolve_runtime_paths(args: argparse.Namespace) -> tuple[Path, Path]:
    ...
    output_path = Path(args.output) if args.output else run_dir / "real_output.json"
    log_path = Path(args.log) if args.log else run_dir / "real_log.tsv"
...
def run_single_project(project_dir: Path, max_steps: int, agent_module) -> tuple[str, str, dict]:
    project_name = project_dir.name
    status = "SUCCESS"
    try:
        result = run_task(project_dir, max_steps, agent_module)
    ...
    return project_name, status, result
...
def main() -> int:
    ...
    output_path, log_path = resolve_runtime_paths(args)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    log_path.parent.mkdir(parents=True, exist_ok=True)
    if not log_path.exists() or log_path.stat().st_size == 0:
        log_path.write_text(
            "timestamp\tproject\tstatus\tduration_s\tprompt_chars\tcompletion_chars\ttool_calls\n"
        )
    ...
    selected_projects = projects[start_idx:end_idx]

    def persist_result(project_name: str, status: str, result: dict) -> None:
    ...
    if workers == 1:
        for project_dir in selected_projects:
            project_name, status, result = run_single_project(project_dir, args.max_steps, agent_module)
            persist_result(project_name, status, result)
    else:
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = [
                executor.submit(run_single_project, project_dir, args.max_steps, agent_module)
                for project_dir in selected_projects
            ]
            for future in as_completed(futures):
                project_name, status, result = future.result()
                persist_result(project_name, status, result)
    return 0
